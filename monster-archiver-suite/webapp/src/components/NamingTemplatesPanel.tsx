...
export default function NamingTemplatesPanel() {
  ...
  const inputClass =
    "w-full bg-[#0b0e14]/60 border border-slate-800 rounded-lg py-2.5 px-4 text-white text-sm font-mono focus:outline-none focus:border-indigo-500 transition-colors";
  ...
  return (
    <div className="bg-[#0f1117] rounded-xl border border-slate-800 p-6 shadow-2xl ring-1 ring-white/5">
      <h3 className="text-white font-medium text-lg mb-5 flex items-center gap-2 border-b border-slate-800/60 pb-3">
        <FolderCog className="w-5 h-5 text-indigo-400" />
        Naming Templates
      </h3>
        ...
        <>
          <div className="space-y-4">
            ...
            <p className="text-slate-600 text-[11px] leading-relaxed">
              Tokens: {TOKENS.map((t) => (
                <code key={t} className="text-indigo-400/80 mr-1.5">
                  {`{${t}}`}
                </code>
              ))}
            </p>

            <div className="flex items-center justify-between gap-4 bg-[#0b0e14]/60 rounded-lg border border-slate-800/60 px-4 py-3">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer border-0 shrink-0 ${
                  values.PRIMARY_ARTIST_BY_FAME ? "bg-indigo-600" : "bg-slate-800"
                }`}
              >
                <span
