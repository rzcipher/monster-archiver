...
// or the finalized captioned copy in the review UI -- same Range-request
// pattern as /api/stream in routes/tags.ts, scoped to video MIME types and
// the same UPLOADS_DIR/OUTPUT_DIR allowlist (the only two places this app
// ever writes a file, so an arbitrary ?path= can't read anything else on disk).
