...
export async function fetchMetadataCandidates(title: string, artist?: string): Promise<any[]> {
  ...
  const [itunesRes, mbRes] = await Promise.allSettled([
    ...
    fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(queryTerm)}&entity=song&limit=5&explicit=Yes`)
      .then(r => r.json())
      .catch(() => ({ results: [] })),
    (async () => {
      await mbThrottle();
      ...
      return fetch(`https://musicbrainz.org/ws/2/recording/?query=recording:"${encodeURIComponent(luceneEscape(cleanTitle))}"${artist ? ` AND artist:"${encodeURIComponent(luceneEscape(String(artist)))}"` : ""}&fmt=json&limit=5`, {
        headers: { "User-Agent": "MonsterArchiverWebSuite/18.4 ( https://github.com/monster-archiver-suite )" }
      })
        .then(r => r.json())
        .catch(() => ({ recordings: [] }));
