...
def extract_audio_info(file_path):
    ...
    try:
        ...
        if audio:
            ...
            if 'itunesadvisory' in audio:
                adv = audio['itunesadvisory'][0]
                if adv == "1": meta["explicit"] = True
                elif adv == "2": meta["explicit"] = False

        if ext == 'mp3':
            try:
                m_aud = MP3(file_path, ID3=ID3)
                if m_aud.tags:
                    for tag in m_aud.tags.getall('TXXX'):
                        if tag.desc.upper() == 'ITUNADVISORY':
                            if tag.text[0] == "1": meta["explicit"] = True
                            elif tag.text[0] == "2": meta["explicit"] = False
            except: pass
        ...
        if not meta["isrc"] and ext in ('m4a', 'aac'):
            try:
                ...
                if m4_raw.tags and 'rtng' in m4_raw.tags:
                    val = m4_raw.tags['rtng'][0]
                    if val == 1: meta["explicit"] = True
                    elif val == 2: meta["explicit"] = False
            except Exception:
                pass
    except Exception: pass
...
def apply_metadata_tags(filepath, data, lyrics_text=None, cover_path=None, cover_mime=None):
    ...
    explicit_val = data.get("explicit")
    ...
    if ext in ("m4a", "aac"):
        try:
            ...
            if explicit_val is True:
                m4_audio.tags["rtng"] = [1]
            elif explicit_val is False:
                m4_audio.tags["rtng"] = [2]
            m4_audio.save()
        except Exception as e:
            sys.stderr.write(f"M4A ISRC/rtng write failed: {e}\n")
    ...
    if ext == "mp3":
        try:
            ...
            if explicit_val is True:
                m_audio.tags.add(TXXX(encoding=3, desc='ITUNADVISORY', text='1'))
            elif explicit_val is False:
                m_audio.tags.add(TXXX(encoding=3, desc='ITUNADVISORY', text='2'))
            m_audio.save()
        except Exception as e:
            sys.stderr.write(f"MP3 ISRC/TXXX write failed: {e}\n")
