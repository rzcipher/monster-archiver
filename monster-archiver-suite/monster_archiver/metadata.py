        ...
        def _fetch_itunes():
            ...
            try:
                r = requests.get(
                    ...
                    # tracks when this param is omitted (storefront-dependent default), so an
                    # explicit song would return zero matches or only a clean/radio edit.
                    f"https://itunes.apple.com/search?term={urllib.parse.quote(clean_query)}&entity=song&limit=3&explicit=Yes",
            ...
            if _real_result_count > 1 and not already_has_single:
                ...
                try:
                    ...
                    itunes_single = requests.get(
                        # explicit=Yes: same Apple quirk as the main search above — without it the
                        # single-artwork lookup finds nothing for explicit tracks.
                        f"https://itunes.apple.com/search?term={urllib.parse.quote(single_query)}&entity=song&limit=1&explicit=Yes",
                        headers={'User-Agent': 'Mozilla/5.0'}, timeout=5
                    ).json()
