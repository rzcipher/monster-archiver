...
// Bind to localhost only — this app is meant to run entirely on your own machine,
// not be exposed to the network. Set HOST=0.0.0.0 in .env if you deliberately want LAN access.
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    // Sanitize the extension itself too — extname() of a hostile originalname
    // can still smuggle control chars or a NUL (e.g. "track.flac\0"), and a
    // NUL in the destination filename makes fs calls throw deep inside multer
    // with a confusing 500. Keep only a short trailing .alnum run instead.
