...
export default function ActivityLog({ open, onClose }: ActivityLogProps) {
  ...
  // Standard overlay affordance: Esc dismisses the drawer just like the X.
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full max-w-md bg-void-900 border-l border-void-700 z-50 flex flex-col"
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-void-700/80">
              <h3 className="text-white font-semibold text-base flex items-center gap-2">
                <History className="w-4.5 h-4.5 text-deezer-400" />
                Recent Activity
              </h3>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
              ...
              {!loading &&
                entries.map((entry) => (
                  <div
                    key={entry.id}
                    className="bg-void-950 rounded-lg p-3.5 border border-void-700/60"
                  >
