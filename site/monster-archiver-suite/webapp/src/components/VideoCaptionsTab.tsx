...
export default function VideoCaptionsTab() {
  ...
  const deleteSegment = (index: number) => {
    ...
    // The "may not have translated" flags are plain segment indices — drop the
    // deleted one and shift every later index down, otherwise a flag silently
    // jumps onto the wrong line (or points past the end of the list).
  ...
  const renameSpeaker = (oldLabel: string, newLabel: string) => {
    ...
    // Renaming onto an existing speaker's name must merge, not duplicate —
    // a naive map left two identical labels in the chip list, and the inputs
    // then both edited the same (first) entry.
    setSpeakers((prev) => {
      if (prev.includes(trimmed)) return prev.filter((sp) => sp !== oldLabel);
      return prev.map((sp) => (sp === oldLabel ? trimmed : sp));
    });
    ...
    <div className="space-y-6">
      ...
      {!uploadedVideo && (
        <div
          ...
          onDrop={handleDrop}
          className={`w-full bg-void-900 rounded-xl border-2 border-dashed p-10 text-center transition-all duration-300 relative overflow-hidden ${
            dragActive ? "border-deezer-400 scale-[1.01]" : "border-void-700 hover:border-deezer-500/50"
          }`}
        >
          <input
            type="file"
          ...
          <label htmlFor="videoInput" className="cursor-pointer block relative">
            <div
              className={`w-16 h-16 bg-void-950 border rounded-xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 text-slate-400 shadow-lg ${
                dragActive ? "scale-110 border-deezer-400/60 text-deezer-300" : "border-void-700"
              }`}
            >
              {uploading ? (
        ...
        <>
          ...
          <div className="bg-void-900 rounded-xl border border-void-700 p-5 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3.5 min-w-0">
              <div className="p-3 bg-deezer-950/30 border border-deezer-800/20 rounded-lg text-deezer-400 shrink-0">
            ...
            <button
              onClick={resetAll}
              className="px-3 py-2 bg-void-950 hover:bg-slate-800 border border-void-700 text-slate-300 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          <div className="bg-void-900 rounded-xl border border-void-700 p-5">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-white font-medium text-sm flex items-center gap-1.5 uppercase tracking-wider text-slate-400">
                <Video className="w-4 h-4 text-deezer-400" />
                Preview
              </h4>
              {finalizedPath && (
                <div className="flex items-center bg-void-950 rounded-full p-0.5 border border-void-700">
                  {(["original", "finalized"] as const).map((mode) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider transition-colors cursor-pointer border-0 ${
                        previewMode === mode
                          ? "bg-gradient-to-r from-deezer-600 to-flow-500 text-white"
          ...
          {!hasResults && (
            <div className="bg-void-900 rounded-xl border border-void-700 p-6">
              <h3 className="text-white font-medium text-lg mb-4 flex items-center gap-2 border-b border-void-700/60 pb-3">
                <Clapperboard className="w-5 h-5 text-deezer-400" />
                Generate Captions
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-void-950 border border-void-700 rounded-lg py-2.5 px-3 text-white text-sm focus:outline-none focus:border-deezer-500 transition-colors cursor-pointer"
                  >
                    {LANG_OPTIONS.map((o) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-void-950 border border-void-700 rounded-lg py-2.5 px-3 text-white text-sm focus:outline-none focus:border-deezer-500 transition-colors"
                  />
                </div>
              </div>
          ...
          {hasResults && (
            <>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* SEGMENTS LIST */}
                <div className="lg:col-span-2 bg-void-900 rounded-xl border border-void-700 p-6">
                  <div className="flex flex-wrap items-center justify-between gap-3 mb-4 border-b border-void-700/60 pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={addSegment}
                        className="px-2.5 py-1.5 bg-void-950 border border-void-700 hover:bg-slate-800 text-slate-300 rounded-lg text-xs flex items-center gap-1 transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-void-950 border border-void-700 hover:bg-slate-800 text-slate-300 rounded-lg text-xs flex items-center gap-1 transition-colors cursor-pointer disabled:opacity-50"
                      >
                        {transcribing ? (
                  ...
                  <div className="space-y-2.5 max-h-[560px] overflow-y-auto custom-scrollbar pr-1">
                    {segments.map((seg, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`bg-void-950 rounded-lg p-3 border transition-colors ${
                            invalid ? "border-rose-800/60" : flagged ? "border-amber-700/50" : "border-void-700/60"
                          }`}
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <button
                              ...
                              className="flex items-center gap-1 text-[11px] font-mono text-slate-400 hover:text-deezer-300 bg-transparent border-0 cursor-pointer px-1"
                            >
                            ...
                            <input
                              ...
                              className="w-16 bg-void-900 border border-void-700 rounded px-1.5 py-0.5 text-[11px] font-mono text-slate-300 focus:outline-none focus:border-deezer-500"
                            />
                            ...
                            <select
                              ...
                              className="ml-auto bg-void-900 border border-void-700 rounded px-2 py-1 text-[11px] font-semibold focus:outline-none focus:border-deezer-500 cursor-pointer"
                              style={{ color }}
                            >
                              {speakers.map((sp) => (
                ...
                <div className="space-y-6">
                  <div className="bg-void-900 rounded-xl border border-void-700 p-6">
                    <h4 className="text-white font-semibold text-sm uppercase tracking-wider mb-4 flex items-center gap-1.5">
                      <Users className="w-4 h-4 text-deezer-400" />
                      Speakers
                    </h4>
                    <div className="space-y-2">
                      {speakers.map((sp) => (
                        <div key={sp} className="flex items-center gap-2 bg-void-950 border border-void-700/60 rounded-lg px-2.5 py-1.5">
                          <span
                    ...
                    <button
                      onClick={addSpeaker}
                      className="w-full mt-3 py-2 bg-void-950 hover:bg-slate-800 border border-void-700 text-slate-300 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                    >
                  ...
                  <div className="bg-void-900 rounded-xl border border-void-700 p-6 relative overflow-hidden">
                    <h4 className="text-white font-semibold text-sm uppercase tracking-wider mb-4 flex items-center gap-1.5">
                      <Sparkles className="w-4 h-4 text-deezer-400" />
                      Local LLM Translator
                    </h4>
                    <p className="text-slate-400 text-xs mb-4">
                      Runs on your GPU via <span className="text-deezer-400 font-semibold font-mono">Ollama</span> — no
                    ...
                    <div className="mb-4">
                      ...
                      <select
                        ...
                        className="w-full bg-void-950 border border-void-700 rounded-lg py-2 px-3 text-white text-xs focus:outline-none focus:border-deezer-500 transition-colors cursor-pointer"
                      >
                    ...
                    <button
                      ...
                      className="w-full py-2.5 bg-void-950 hover:bg-slate-800 border border-deezer-500/40 text-deezer-300 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                    >
                      {translating ? (
                        <>
                          <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                  ...
                  <div className="bg-void-900 rounded-xl border border-void-700 p-6">
                    ...
                    <div className="mb-4">
                      ...
                      <input
                        ...
                        className="w-full accent-deezer-500 cursor-pointer"
                      />
                    </div>
                    ...
                    {downloadUrl ? (
                      <div className="space-y-2">
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-void-950 hover:bg-slate-800 border border-void-700 text-slate-300 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                        >
                          {burning ? (
                            <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                          ) : (
