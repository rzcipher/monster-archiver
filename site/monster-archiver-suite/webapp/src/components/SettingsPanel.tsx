...
function FieldInput({
  ...
  const baseClass =
    "w-full bg-void-950 border border-void-700 rounded-lg py-2.5 px-4 text-white text-sm focus:outline-none focus:border-deezer-500 transition-colors";
    ...
    return (
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer border-0 shrink-0 ${
          value ? "bg-deezer-600" : "bg-slate-800"
        }`}
      >
        <span
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <RefreshCw className="w-8 h-8 text-deezer-400 animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {SETTINGS_SECTIONS.map((section) => (
        <div key={section} className="bg-void-900 rounded-xl border border-void-700 p-6">
          <h3 className="text-white font-medium text-lg mb-5 flex items-center gap-2 border-b border-void-700/60 pb-3">
            <SlidersHorizontal className="w-5 h-5 text-deezer-400" />
            {section}
          </h3>
      ...
      <div className="sticky bottom-6 flex justify-end">
        <button
          ...
          className="px-6 py-3 bg-gradient-to-r from-deezer-600 to-flow-500 hover:from-deezer-500 hover:to-flow-400 disabled:from-slate-800 disabled:to-slate-800 disabled:text-slate-600 text-white rounded-lg text-sm font-semibold transition-all shadow-lg flex items-center gap-2 cursor-pointer border-0"
        >
          {saving ? (
            <>
