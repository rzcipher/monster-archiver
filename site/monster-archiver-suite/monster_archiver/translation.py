...
methods "1"/"2"/"3"), a local Ollama LLM engine (method "5"), method "6"
(Google Gemini Pro, web-app only — the CLI warns and skips), the ASCII
    ...
    try:
        ...
        if texts_to_translate and state.GLOBAL_TRANS_METHOD != "0":
            if state.GLOBAL_TRANS_METHOD == "5":
                ...
                elif state.GLOBAL_TRANS_METHOD == "6":
                    # Method 6 (Google Gemini Pro) is implemented by the web
                    # app's server (webapp/lib/geminiTranslate.ts), not here —
                    # without this explicit branch the dispatcher silently
                    # skipped the sleep-only else-chain and users wondered why
                    # "configured engine" did nothing in CLI runs.
                    ui.log("[Translation] Engine 6 (Google Gemini Pro) is only wired up in the web app; skipping translation for this track. Choose 1, 2, 3 or 5 for CLI runs.", "bold yellow")
